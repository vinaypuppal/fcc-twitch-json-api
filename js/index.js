...
$(document).ready(function () {
  ...
  var streamRequests = streamers.map(function (item) {
    return axios.get(
      `${streamUrl}${item}?client_id=ac7y4kusg6ljy3317y4cy8hiqknxtj3`,
      {
  ...
  var userRequests = streamers.map(function (item) {
    return axios.get(
      `${userUrl}${item}?client_id=ac7y4kusg6ljy3317y4cy8hiqknxtj3`,
      {
        validateStatus: function (status) {
          return status < 500 // Reject only if the status code is greater than or equal to 500
        }
  ...
  axios
    ...
    .then(function (streamResponse) {
      axios.all(userRequests).then(function (userResponse) {
        $('.loading').hide()
  ...
  function seperateData (streamResults, userResults) {
    var unknownStreamResults = streamResults
    ...
    for (var i = 0; i < unknownStreamResults.length; i++) {
      let temp = unknownStreamResults[i]
  ...
  function generateCard (data) {
    var logo = data.user.logo
      ? data.user.logo
      : 'https://placeholdit.imgix.net/~text?txtsize=14&txt=Logo&w=50&h=50'
    if (data.status === 422) {
      var state = 'Account Closed'
    } else {
      var state = data.stream ? 'online' : 'offline'
    }
    var bioUrl = data.user.bio
      ? 'https://placeholdit.imgix.net/~text?txtsize=15&txt=' +
          data.user.bio +
          '&w=300&h=200'
      : 'https://placeholdit.imgix.net/~text?txtsize=160&txt=?&w=300&h=200'
    ...
    var displayName = data.user.display_name
      ? data.user.display_name
      : data.message
    return (
      '<li class="animated zoomIn"><a href="' +
      twitchUrl +
      '" class="card"  target="_blank" ><div class="card-header"><div class="left"><img src="' +
      logo +
      '" alt="logo" /></div><div class="right"><div class="top">' +
      displayName +
      ...
      state[0].toUpperCase() +
      state.slice(1) +
      '</div></div></div><div class="card-body"><img src="' +
      previewUrl +
      '" alt="preview" /></div><div class="card-footer"><div class="top"><div class="left"><span>Lang</span>: ' +
      lang +
      '</div><div class="right"><span>Viewers</span>: ' +
      viewers +
      '</div></div><div class="bottom"><span>Playing</span>: ' +
      playing +
      '</div></div></a></li>'
    )
  }
